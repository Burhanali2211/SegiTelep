...
export const TextSegmentEditor = memo<TextSegmentEditorProps>(({ className }) => {
  ...
  // If it's not a text segment, show a message
  if (segment.type !== 'text') {
    return (
      <div className={cn('flex items-center justify-center h-full', className)}>
        <div className="text-center text-muted-foreground">
          <Type size={48} className="mx-auto mb-4 opacity-50" />
          <p>This is not a text segment</p>
          <p className="text-sm mt-2">Select a text segment to edit its content</p>
        </div>
      </div>
    );
  }
...
TextSegmentEditor.displayName = 'TextSegmentEditor';
