...
export const ImageCanvas = memo<ImageCanvasProps>(({ className }) => {
  ...
  const { url: assetUrl, isLoading: isAssetLoading } = useAssetUrl(currentPage?.assetId, currentPage?.data);

  // Load image when URL changes - capture natural dimensions
  useEffect(() => {
    if (!assetUrl) {
    ...
    setImageLoaded(false); // Reset while loading new src
    const img = new Image();
    img.onload = () => {
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      // Add margin for segment handles (16px on each side)
      const margin = 32;
      const displayWidth = Math.max(100, rect.width - margin);
